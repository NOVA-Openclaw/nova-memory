...
echo "$JSON_DATA" | jq -c '.opinions[]? // empty' | while read -r opinion; do
    ...
    echo "INSERT INTO entity_facts (entity_id, key, value, source)
          SELECT id, 'opinion_$(sql_escape "$subject")', '$(sql_escape "$opinion_text")', 'auto-extracted from $holder'
          FROM entities WHERE name = '$(sql_escape "$holder")'
          ON CONFLICT DO NOTHING;" | psql -h "$DB_HOST" -U "$DB_USER" -d "$DB" -q 2>/dev/null || true
    echo "  + Opinion: $holder thinks '$opinion_text' about $subject"
done
